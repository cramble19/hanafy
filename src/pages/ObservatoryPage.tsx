...
} from '@/components/CrambleQuestHubPanel'
import {
  QuestHubTabs,
  questHubPanelId,
  questHubTabId,
    ...
    <div className="cramble-archive-shell cramble-observatory-shell mx-auto min-h-full w-full max-w-md overflow-hidden px-5 pb-10 pt-6 text-white">
      ...
      <div className="relative z-10 mb-6 flex items-center justify-between gap-3">
        ...
        <h1
          ref={headingRef}
          tabIndex={-1}
          className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/80 outline-none backdrop-blur"
        >
          Lantern Observatory
        </h1>
      </div>
      ...
      <section
        id={questHubPanelId('cramble', 'destination')}
        role="tabpanel"
        aria-labelledby={questHubTabId('cramble', 'destination')}
        hidden={view !== 'destination'}
      >
      ...
      <section
        id={questHubPanelId('cramble', 'quests')}
        role="tabpanel"
        aria-labelledby={questHubTabId('cramble', 'quests')}
        hidden={view !== 'quests'}
      >
        <CrambleQuestHubPanel game={game} {...questActions} />
      </section>
    </div>
  )
}
